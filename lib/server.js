...
exports.run = function(config, ready) {
    ...
    server.on("connect", function(client) {
        ...
        client.on("authenticate", function(opts, cb) {
            function findUser(jid, password, cb){
                User.find(jid, function(user) {
                    if (user && user.attrs.password === password){
                        console.log("Authentication success for jid " + opts.jid);
                        cb(null, opts);
                    } else {
                        console.log("Authentication failure for jid " + opts.jid);
                        cb(new Error("Authentication failure"));
                    }
                });
            }
            //  Authentication from notebook sends a jid in a special way.
            var auth_token = opts.password;
            request.get('http://localhost:4455/api/user?access_token=' + auth_token, function(error, response, body) {
                if (error || response.statusCode == 401) {
                    ...
                    User.register(user.email, user.user_id, {
                        force: true,
                        success: function() {
                            console.log("Success.");
                            findUser(user.email, user.user_id, cb);
                        },
                        error: function() {
                            findUser(user.email, user.user_id, cb);
                        }
